...
public class BuilderEntityExisting extends ABuilderEntityBase {
    public static EntityType<BuilderEntityExisting> E_TYPE2;
    ...
    /**
     * Max radius of any builder entity in the world.  Forge tracked this on the Level via
     * getMaxEntityRadius/increaseMaxEntityRadius and used it to inflate AABB-based entity queries,
     * but Fabric has no such hook, so we track it here.  Anything doing AABB entity queries that
     * need to find builders should inflate their query box by this amount.  2 blocks is the vanilla default.
     **/
    public static double worldMaxEntityRadius = 2.0D;
            ...
            if (!entity.isValid) {
                ...
                if (entity instanceof AEntityE_Interactable) {
                    ...
                    if (entity instanceof EntityVehicleF_Physics && interactable.ticksExisted > 1 && interactable.ticksExisted % 20 == 0) {
                        ...
                        double maxEntityRadius = worldMaxEntityRadius;
                        if (maxEntityRadius < interactable.encompassingBox.widthRadius || maxEntityRadius < interactable.encompassingBox.heightRadius || maxEntityRadius < interactable.encompassingBox.depthRadius) {
                            ...
                            if (maxEntityRadius < AEntityE_Interactable.MAX_ENTITY_RADIUS) {
                                worldMaxEntityRadius = maxEntityRadius;
                            } else {
    ...
    /**
     * Called when this builder is removed from the world for any reason, including chunk unloading.
     * Forge called this method for us via its Entity extensions, but on Fabric this needs to be
     * wired to ServerEntityEvents.ENTITY_UNLOAD and ClientEntityEvents.ENTITY_UNLOAD.
     */
    ...
    /**
     * We need to hook into explosions here as we don't know where explosions occur in the world.
     ...
     * Called server-side by {@link mcinterfacefabric1201.mixin.common.ExplosionMixin} before the explosion damages entities.
     * The position is passed-in separately since the x/y/z fields on the explosion are private with no getters in vanilla.
     */
    public static void onExplosion(Explosion explosion, double x, double y, double z) {
        lastExplosionPosition = new Point3D(x, y, z);
    }
}
