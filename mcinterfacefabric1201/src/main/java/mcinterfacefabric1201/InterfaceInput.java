...
public class InterfaceInput implements IInterfaceInput {
    ...
    public static void registerKeyMappings() {
        ...
        KeyBindingHelper.registerKeyBinding(importKey);
    }
    ...
    public static void onIVMovementInput(Input input) {
        if (InterfaceManager.clientInterface != null && ControlSystem.shouldSuppressDismount(InterfaceManager.clientInterface.getClientPlayer(), input.shiftKeyDown)) {
     ...
     */
    public static void onIVKeyInput(int key, int action) {
    ...
    public static void registerScreenEvents() {
        ScreenEvents.AFTER_INIT.register((client, screen, scaledWidth, scaledHeight) -> ScreenMouseEvents.afterMouseScroll(screen).register((scrolledScreen, mouseX, mouseY, horizontalAmount, verticalAmount) -> {
            if (InterfaceManager.clientInterface.isGUIOpen()) {
                lastScrollValue = (int) verticalAmount;
            }
        }));
    }
}
